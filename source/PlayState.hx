...
class PlayState extends MusicBeatState
{
	...
	var codeBgOne:FlxSprite;
	var codeBgTwo:FlxSprite;
	var rockFloat:FlxSprite;
	...
	override public function create()
	{
					...
					case 'studio':
						{
							defaultCamZoom = 0.9;
							curStage = 'studio';
							var bg:FlxSprite = new FlxSprite(-125, -45).loadGraphic(Paths.image('robo/studio', 'shared'));
							if(FlxG.save.data.antialiasing)
								{
									bg.antialiasing = true;
								}
						...
						case 'codehell':
							...
							defaultCamZoom = 0.35;
							curStage = 'codehell';
							var bg:FlxSprite = new FlxSprite(-50, -625).loadGraphic(Paths.image('robo/CODEBG', 'shared'));
							...
							var codeBgOne:FlxSprite = new FlxSprite(750,0);
							codeBgOne.frames = Paths.getSparrowAtlas('robo/RainCode1', 'shared');
							codeBgOne.animation.addByPrefix('idle','raincode1');
							codeBgOne.animation.play('idle');
							codeBgOne.setGraphicSize(Std.int(codeBgOne.width * 2.5));
							codeBgOne.screenCenter();
							if(FlxG.save.data.antialiasing)
								{
									codeBgOne.antialiasing = true;
								}
							add(codeBgOne);

							
							var codeBgTwo:FlxSprite = new FlxSprite(750,0);
							codeBgTwo.frames = Paths.getSparrowAtlas('robo/RainCode2', 'shared');
							codeBgTwo.animation.addByPrefix('idle','raincode2');
							codeBgTwo.animation.play('idle');
							codeBgTwo.setGraphicSize(Std.int(codeBgTwo.width * 2.5));
							codeBgTwo.screenCenter();
							if(FlxG.save.data.antialiasing)
								{
									codeBgTwo.antialiasing = true;
								}
							add(codeBgTwo);
							}
		...
		switch (curStage)
			{
			case 'codehell':
				gf = new Character(400, -100, curGf);
				gf.scrollFactor.set(0.95, 0.95);
				FlxTween.tween(gf, {y:-10}, 1, {type: FlxTweenType.PINGPONG, ease: FlxEase.quadInOut, loopDelay: 0.1});
			default:
		...
		{
			case 'codehell':
				boyfriend = new Boyfriend(770,  130, SONG.player1);
				FlxTween.tween(boyfriend, {y: 220}, 1, {type: FlxTweenType.PINGPONG, ease: FlxEase.quadInOut, loopDelay: 0.1});
			default:
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			switch (curStage)
			{
				case 'studio':
					...
					add(boyfriend);
				case 'codehell':
					var rockFloat:FlxSprite = new FlxSprite(675, 425).loadGraphic(Paths.image('robo/floating_rock', 'shared'));
					rockFloat.setGraphicSize(Std.int(rockFloat.width * 1));
					if(FlxG.save.data.antialiasing)
						{
							rockFloat.antialiasing = true;
						}
					rockFloat.scrollFactor.set(0.9, 0.9);
					rockFloat.active = false;
					FlxTween.tween(rockFloat, {y:515}, 1, {type: FlxTweenType.PINGPONG, ease: FlxEase.quadInOut, loopDelay: 0.1});	
					add(dad);
					add(rockFloat);
					add(gf);
					add(boyfriend);
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					{
						case 'overload':
							LoadingState.loadAndSwitchState(new VideoState("assets/videos/cutsceneOverload.webm", new PlayState()));
						default:
	...
	override function beatHit()
	{
		...
		if (FlxG.save.data.camzoom)
		{
			...
			if (curSong.toLowerCase() == 'overload' && curBeat >= 32 && curBeat < 224 && camZooming && FlxG.camera.zoom < 1.35)
				{
					FlxG.camera.zoom += 0.015;
					camHUD.zoom += 0.05;
				}
			if (curSong.toLowerCase() == 'overload' && curBeat >= 256 && curBeat < 444 && camZooming && FlxG.camera.zoom < 1.35)
				{
		...
		if (curBeat == 383 && curSong == 'overload')
			{
				boyfriend.playAnim('hey', true);
			}
