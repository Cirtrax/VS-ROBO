...
class CachedFrames
{
    ...
    {
        var graph = FlxGraphic.fromAssetKey(Paths.image(path,'robo'));
        graph.persist = true;
    ...
    public function loadFrames()
    {
        sys.thread.Thread.create(() -> {
            toBeLoaded.set('left','robo/big robo assets right and left');
            toBeLoaded.set('right','robo/big robo assets right and left');
            toBeLoaded.set('up','robo/big robo assets up and down');
            toBeLoaded.set('down','robo/big robo assets up and down');
            toBeLoaded.set('idle','robo/big robo assets idle');
            // all the big sprites
            var numba = 0;
